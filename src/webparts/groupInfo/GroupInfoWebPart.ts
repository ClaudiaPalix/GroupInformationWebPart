...
import styles from './GroupInfoWebPart.module.scss';
import * as strings from 'GroupInfoWebPartStrings';

import { SPHttpClient, SPHttpClientResponse } from '@microsoft/sp-http';

export interface IGroupInfoWebPartProps {
  description: string;
  selectedList: string; 
  policiesUrl: string;
  Title: string;
...
export default class GroupInfoWebPart extends BaseClientSideWebPart<IGroupInfoWebPartProps> {
  ...
  private _renderButtons(): void {
    ...
    .then(data => {
      ...
      if (data.value && data.value.length > 0) {
        data.value.forEach((item: IGroupInfoWebPartProps) => {
          const buttonDiv: HTMLDivElement = document.createElement('div');
          buttonDiv.classList.add(styles['content-box']); // Apply the 'content-box' class styles from YourStyles.module.scss
          
          if (item.Title === 'Policies' || item.Title === 'policies') { 
            if(this.userEmail.includes("_zensar.com#EXT#")){
              buttonDiv.onclick = (event) => {
                if(this.properties.policiesUrl.includes(siteUrl)){
                  event.preventDefault(); // Prevent the default behavior of the click event
                  window.location.href = this.properties.policiesUrl;
                }else{
                  window.open(this.properties.policiesUrl, '_blank'); // Open the 'Url' from the API response in a new tab
                }
              };
            }else{
              buttonDiv.onclick = async () => {
              await this.loadChatterOnScript();
              this.loadChatbot("I want to learn more about policies");
              };
            }
          } else {
            buttonDiv.onclick = (event) => {
  ...
  protected onPropertyPaneFieldChanged(propertyPath: string, oldValue: any, newValue: any): void {
    ...
    else if (propertyPath === 'policiesUrl') {
      this.properties.policiesUrl = newValue;
    }
          ...
          groups: [
            {
              ...
              groupFields: [
                PropertyPaneTextField('description', {
                  label: "Title For The Webpart"
                }),
                PropertyPaneDropdown('selectedList', {
                  label: 'Select A List',
                  options: this.availableLists,
                }),
                PropertyPaneTextField('policiesUrl', {
                  label: "Url For Policies(for zensar users)"
                }),
              ],
            },
          ],
